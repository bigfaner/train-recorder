...
const COLUMNS = [
  'id', 'biz_key', 'exercise_biz_key', 'pr_type',
  'pr_value', 'pr_date', 'workout_set_biz_key', 'created_at',
];
...
export interface PersonalRecordRepo extends BaseRepo<PersonalRecord> {
  ...
  findByType(exerciseBizKey: bigint, prType: PersonalRecord['pr_type']): PersonalRecord[];
  findMaxByExercise(exerciseBizKey: bigint, prType: PersonalRecord['pr_type']): PersonalRecord | null;
}

export function createPersonalRecordRepo(db: DatabaseAdapter): PersonalRecordRepo {
