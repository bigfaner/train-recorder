...
const COLUMNS = [
  'id', 'biz_key', 'session_date', 'training_type', 'session_status',
  'started_at', 'ended_at', 'is_backlog', 'created_at', 'updated_at',
];
...
export interface WorkoutSessionRepo extends BaseRepo<WorkoutSession> {
  ...
  findByTrainingType(trainingType: WorkoutSession['training_type']): WorkoutSession[];
  completeSession(id: number): WorkoutSession;
  partialCompleteSession(id: number): WorkoutSession;
  findActive(): WorkoutSession | null;
}

export function createWorkoutSessionRepo(db: DatabaseAdapter): WorkoutSessionRepo {
  const base = createBaseRepository<WorkoutSession>(db, TABLE_NAME, COLUMNS);
  const columnsStr = COLUMNS.join(', ');

  function transitionStatus(id: number, newStatus: WorkoutSession['session_status']): WorkoutSession {
