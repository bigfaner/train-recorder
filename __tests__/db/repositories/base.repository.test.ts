...
import type { TrainingPlan, TrainingDay, Exercise, PlanExercise } from '../../../src/types';
...
describe('BaseRepository', () => {
  ...
  it('should find all records', () => {
    ...
    const now = new Date().toISOString();
    repo.create({
      biz_key: 3001n, plan_name: 'Plan A', plan_mode: 'infinite_loop',
      cycle_length: null, schedule_mode: 'weekly_fixed', rest_days: 1,
      weekly_config: null, is_active: 0, created_at: now, updated_at: now,
    });
    repo.create({
      biz_key: 3002n, plan_name: 'Plan B', plan_mode: 'fixed_cycle',
      cycle_length: 8, schedule_mode: 'weekly_fixed', rest_days: 1,
      weekly_config: null, is_active: 0, created_at: now, updated_at: now,
    });
  ...
  it('should update a record', () => {
    ...
    const now = new Date().toISOString();
    const plan = repo.create({
      biz_key: 4001n, plan_name: 'Original', plan_mode: 'infinite_loop',
      cycle_length: null, schedule_mode: 'weekly_fixed', rest_days: 1,
      weekly_config: null, is_active: 0, created_at: now, updated_at: now,
    });
  ...
  it('should delete a record', () => {
    ...
    const now = new Date().toISOString();
    const plan = repo.create({
      biz_key: 5001n, plan_name: 'To Delete', plan_mode: 'infinite_loop',
      cycle_length: null, schedule_mode: 'weekly_fixed', rest_days: 1,
      weekly_config: null, is_active: 0, created_at: now, updated_at: now,
    });
...
describe('TrainingPlanRepository', () => {
  ...
  it('should activate a plan and deactivate others', () => {
    ...
    const planA = repo.createPlan({
      biz_key: 101n, plan_name: 'Plan A', plan_mode: 'infinite_loop',
      cycle_length: null, schedule_mode: 'weekly_fixed', rest_days: 1,
      weekly_config: null, is_active: 1, created_at: now, updated_at: now,
    });
    const planB = repo.createPlan({
      biz_key: 102n, plan_name: 'Plan B', plan_mode: 'infinite_loop',
      cycle_length: null, schedule_mode: 'weekly_fixed', rest_days: 1,
      weekly_config: null, is_active: 0, created_at: now, updated_at: now,
    });
  ...
  it('should find the active plan', () => {
    ...
    repo.createPlan({
      biz_key: 103n, plan_name: 'Inactive', plan_mode: 'infinite_loop',
      cycle_length: null, schedule_mode: 'weekly_fixed', rest_days: 1,
      weekly_config: null, is_active: 0, created_at: now, updated_at: now,
    });
    repo.createPlan({
      biz_key: 104n, plan_name: 'Active', plan_mode: 'infinite_loop',
      cycle_length: null, schedule_mode: 'weekly_fixed', rest_days: 1,
      weekly_config: null, is_active: 1, created_at: now, updated_at: now,
    });
  ...
  it('should deactivate a plan', () => {
    ...
    const plan = repo.createPlan({
      biz_key: 105n, plan_name: 'Active Plan', plan_mode: 'infinite_loop',
      cycle_length: null, schedule_mode: 'weekly_fixed', rest_days: 1,
      weekly_config: null, is_active: 1, created_at: now, updated_at: now,
    });
...
describe('TrainingDayRepository', () => {
  ...
  it('should find days by plan_biz_key', () => {
    ...
    repo.createDay({
      biz_key: 201n, plan_biz_key: planBizKey, day_name: 'Push',
      training_type: 'push', order_index: 0, created_at: now, updated_at: now,
    });
    repo.createDay({
      biz_key: 202n, plan_biz_key: planBizKey, day_name: 'Pull',
      training_type: 'pull', order_index: 1, created_at: now, updated_at: now,
    });
    repo.createDay({
      biz_key: 203n, plan_biz_key: 999n, day_name: 'Other',
      training_type: 'custom', order_index: 0, created_at: now, updated_at: now,
    });
  ...
  it('should update a training day', () => {
    ...
    const now = new Date().toISOString();
    const day = repo.createDay({
      biz_key: 204n, plan_biz_key: 100n, day_name: 'Original',
      training_type: 'push', order_index: 0, created_at: now, updated_at: now,
    });
...
describe('ExerciseRepository', () => {
  ...
  it('should findAll exclude soft-deleted exercises by default', () => {
    ...
    repo.createExercise({
      biz_key: 302n, exercise_name: 'Active', category: 'core_powerlifting',
      increment: 2.5, default_rest: 300, is_custom: 0, is_deleted: 0,
      created_at: now, updated_at: now,
    });
    const deleted = repo.createExercise({
      biz_key: 303n, exercise_name: 'Deleted', category: 'core_powerlifting',
      increment: 2.5, default_rest: 300, is_custom: 0, is_deleted: 0,
      created_at: now, updated_at: now,
    });
  ...
  it('should find exercises by category', () => {
    ...
    repo.createExercise({
      biz_key: 304n, exercise_name: 'Squat', category: 'core_powerlifting',
      increment: 2.5, default_rest: 300, is_custom: 0, is_deleted: 0,
      created_at: now, updated_at: now,
    });
    repo.createExercise({
      biz_key: 305n, exercise_name: 'Pull-up', category: 'upper_pull',
      increment: 2.5, default_rest: 240, is_custom: 0, is_deleted: 0,
      created_at: now, updated_at: now,
    });
  ...
  it('findByCategory should exclude soft-deleted', () => {
    ...
    const ex = repo.createExercise({
      biz_key: 306n, exercise_name: 'Deadlift', category: 'core_powerlifting',
      increment: 2.5, default_rest: 300, is_custom: 0, is_deleted: 0,
      created_at: now, updated_at: now,
    });
  ...
  it('should update an exercise', () => {
    ...
    const now = new Date().toISOString();
    const exercise = repo.createExercise({
      biz_key: 307n, exercise_name: 'OHP', category: 'shoulder',
      increment: 2.5, default_rest: 180, is_custom: 0, is_deleted: 0,
      created_at: now, updated_at: now,
    });
...
describe('PlanExerciseRepository', () => {
  ...
  it('should find by training_day_biz_key', () => {
    ...
    repo.createPlanExercise({
      biz_key: 401n, training_day_biz_key: dayBizKey, exercise_biz_key: 300n,
      sets_config: '{"mode":"fixed","target_reps":5,"target_weight":80,"target_repeat":5}',
      order_index: 0, exercise_note: null, created_at: now, updated_at: now,
    });
    repo.createPlanExercise({
      biz_key: 402n, training_day_biz_key: dayBizKey, exercise_biz_key: 301n,
      sets_config: '{"mode":"fixed","target_reps":8,"target_weight":60,"target_repeat":3}',
      order_index: 1, exercise_note: null, created_at: now, updated_at: now,
    });
    repo.createPlanExercise({
      biz_key: 403n, training_day_biz_key: 999n, exercise_biz_key: 302n,
      sets_config: '{"mode":"fixed","target_reps":5,"target_weight":100,"target_repeat":5}',
      order_index: 0, exercise_note: null, created_at: now, updated_at: now,
    });
  ...
  it('should update a plan exercise', () => {
    ...
    const now = new Date().toISOString();
    const pe = repo.createPlanExercise({
      biz_key: 405n, training_day_biz_key: 203n, exercise_biz_key: 304n,
      sets_config: '{"mode":"fixed","target_reps":5,"target_weight":80,"target_repeat":5}',
      order_index: 0, exercise_note: null, created_at: now, updated_at: now,
    });

    const newConfig = JSON.stringify({ mode: 'fixed', target_reps: 3, target_weight: 100, target_repeat: 5 });
    const updated = repo.update(pe.id, { sets_config: newConfig, exercise_note: 'Heavy day' });
