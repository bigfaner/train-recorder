...
import {
  createFeelingRepo,
  type FeelingRepo,
} from "../../../src/db/repositories/feeling.repo";
import {
  createExerciseFeelingRepo,
  type ExerciseFeelingRepo,
} from "../../../src/db/repositories/exercise-feeling.repo";
import {
  createPersonalRecordRepo,
  type PersonalRecordRepo,
} from "../../../src/db/repositories/personal-record.repo";
import {
  createBodyMeasurementRepo,
  type BodyMeasurementRepo,
} from "../../../src/db/repositories/body-measurement.repo";
import {
...
import {
  createSportTypeRepo,
  type SportTypeRepo,
} from "../../../src/db/repositories/sport-type.repo";
import {
  createSportMetricRepo,
  type SportMetricRepo,
} from "../../../src/db/repositories/sport-metric.repo";
import {
  createSportMetricValueRepo,
  type SportMetricValueRepo,
} from "../../../src/db/repositories/sport-metric-value.repo";
import {
...
describe("FeelingRepository", () => {
  ...
  function createTestFeeling(
    overrides: Partial<Omit<Feeling, "id">> = {},
  ): Feeling {
...
describe("ExerciseFeelingRepository", () => {
  ...
  function createTestExerciseFeeling(
    overrides: Partial<Omit<ExerciseFeeling, "id">> = {},
  ): ExerciseFeeling {
    const now = new Date().toISOString();
    efBizKeyCounter += 1n;
  ...
  describe("findByWorkoutExerciseBizKey", () => {
    it("should find exercise feeling by workout_exercise_biz_key", () => {
      createTestExerciseFeeling({
        biz_key: 4020n,
        workout_exercise_biz_key: 6001n,
      });
      createTestExerciseFeeling({
        biz_key: 4021n,
        workout_exercise_biz_key: 6002n,
      });
...
describe("PersonalRecordRepository", () => {
  ...
  function createTestPR(
    overrides: Partial<Omit<PersonalRecord, "id">> = {},
  ): PersonalRecord {
    const now = new Date().toISOString();
    prBizKeyCounter += 1n;
...
describe("BodyMeasurementRepository", () => {
  ...
  function createTestBodyMeasurement(
    overrides: Partial<Omit<BodyMeasurement, "id">> = {},
  ): BodyMeasurement {
    const now = new Date().toISOString();
    bmBizKeyCounter += 1n;
  ...
  describe("findByDateRange", () => {
    it("should find measurements within date range inclusive", () => {
      ...
      const results = bodyMeasurementRepo.findByDateRange(
        "2026-05-05",
        "2026-05-10",
      );
      expect(results.length).toBe(2);
    ...
    it("should return empty array for range with no measurements", () => {
      expect(
        bodyMeasurementRepo.findByDateRange("2020-01-01", "2020-12-31"),
      ).toEqual([]);
    });
  });
...
describe("OtherSportRecordRepository", () => {
  ...
  describe("CRUD", () => {
    ...
    it("should update an other sport record", () => {
      ...
      const updated = otherSportRepo.update(osr.id, {
        sport_note: "Great run",
      });
...
describe("SportTypeRepository", () => {
  ...
  function createTestSportType(
    overrides: Partial<Omit<SportType, "id">> = {},
  ): SportType {
    const now = new Date().toISOString();
    stBizKeyCounter += 1n;
...
describe("SportMetricRepository", () => {
  ...
  function createTestSportMetric(
    overrides: Partial<Omit<SportMetric, "id">> = {},
  ): SportMetric {
    smBizKeyCounter += 1n;
  ...
  describe("findBySportTypeBizKey", () => {
    it("should find metrics by sport_type_biz_key ordered by order_index", () => {
      createTestSportMetric({
        biz_key: 9010n,
        sport_type_biz_key: 5001n,
        order_index: 2,
      });
      createTestSportMetric({
        biz_key: 9011n,
...
describe("SportMetricValueRepository", () => {
  ...
  function createTestSportMetricValue(
    overrides: Partial<Omit<SportMetricValue, "id">> = {},
  ): SportMetricValue {
    smvBizKeyCounter += 1n;
  ...
  describe("CRUD", () => {
    ...
    it("should update a sport metric value", () => {
      ...
      const updated = sportMetricValueRepo.update(smv.id, {
        metric_value: 10.5,
      });
  ...
  describe("findBySportRecordBizKey", () => {
    it("should find metric values by sport_record_biz_key", () => {
      createTestSportMetricValue({
        biz_key: 10010n,
        sport_record_biz_key: 7001n,
      });
      createTestSportMetricValue({
        biz_key: 10011n,
        sport_record_biz_key: 7001n,
  ...
  describe("getValue / setValue (upsert pattern)", () => {
    ...
    it("should handle JSON serialized values", () => {
      const jsonValue = JSON.stringify({
        skipped_dates: ["2026-05-01", "2026-05-02"],
      });
...
describe("Other sport entity integration", () => {
  it("should support full other sport flow: type -> metric -> record -> values", () => {
    ...
    const sportRecords = otherSportRepo.findBySportTypeBizKey(
      sportType.biz_key,
    );
    expect(sportRecords.length).toBe(1);
    expect(sportRecords[0].sport_note).toBe("Great swim session");

    const metricValues = sportMetricValueRepo.findBySportRecordBizKey(
      record.biz_key,
    );
