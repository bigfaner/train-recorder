...
function createMockDatabase() {
  const settings: Map<string, { setting_value: string; updated_at: string }> =
    new Map();
  ...
  const db = {
    ...
    runSync(
      ...
      if (sql.includes("INSERT OR REPLACE INTO user_settings")) {
        const _bizKey = params?.[0] as number;
      ...
      if (sql.includes("CREATE TABLE")) {
        ...
        settingsTableCreated =
          settingsTableCreated || sql.includes("user_settings");
        return { lastInsertRowId: 1, changes: 1 };
      }
      return { lastInsertRowId: 1, changes: 1 };
    },
...
type _MockDatabase = ReturnType<typeof createMockDatabase>;
...
describe("Migration Manager - getCurrentVersion", () => {
  ...
  it("should return the stored version number", () => {
    ...
    db._settings.set(SCHEMA_VERSION_KEY, {
      setting_value: "3",
...
describe("Migration Manager - isDatabaseInitialized", () => {
  it("should return false when user_settings table does not exist", () => {
    ...
    const result = isDatabaseInitialized(
      db as unknown as import("expo-sqlite").SQLiteDatabase,
    );
